	...
	class User{
		...
		public function create($fields = array()){
			...
			public function find( $user = null){
				...
				if($user){
					$field = (is_numeric($user)) ? 'staff_id' : 'username';
					...
					Session::put($this->_sessionName, $this->data()->staff_id);
						...
						$user = $this->find($username);
							...
							if ($this->data()->password === Hash::make($password, $this->data()->salt)) {
							 ...
							 if ($remember) {
							 	$hash = Hash::unique();
							 	$hashCheck = $this->_db->get('users_session', array('user_id', '=', $this->data()->staff_id));
							 		if (!$hashCheck->count()) {
							 			# code...

							 			$this->_db->insert('users_session', array(
							 				'user_id' => $this->data()->staff_id,
							 				'hash' => $hash
							 			));
							 		}else{
		...
		public function logout(){
			$this->_db->delete('users_session', array('user_id', '=', $this->data()->staff_id));
